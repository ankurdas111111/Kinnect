<script>
  ...
  let hasAny = ($derived($myRooms.length > 0 || $myContacts.length > 0));
...
{#if embedded}
  <div class="panel-body sharing-root">
    ...
    <div class="quick-actions-row">
      <button class="quick-action-card tactile" class:quick-action-active={$rideShare.active} onclick={() => rideShareOpen = true}>
      ...
      <button class="quick-action-card tactile" onclick={onMyWay}>
      ...
      <button class="quick-action-card qa-invite tactile" onclick={() => inviteOpen = true}>
    ...
    <div class="sharing-section">
      ...
      {#if $myRooms.length === 0}
        <EmptyState
          title="No groups yet"
          body="Create a group above and share the code with your family to see everyone on one map."
        >
          {#snippet icon()}
        ...
        <div class="rooms-list">
          {#each $myRooms as room}
            <Card variant="glass" glow="primary" padding="none">
              <div class="room-card-inner animate-slide-up" style="border-top: 3px solid {getUserColor(room.code)};">
                <div class="room-card-header">
                  ...
                  <button class="btn btn-danger btn-sm tactile" onclick={() => leaveRoom(room.code)} disabled={busyAction === 'leave-' + room.code}>{busyAction === 'leave-' + room.code ? 'Leaving...' : 'Leave Group'}</button>
                </div>

                {#if (room.members || []).length > 0}
                  <div class="room-members-row">
                    {#each (room.members || []) as m}
                      <button class="room-member-chip" onclick={() => locateContact(m.userId)} title="Find {m.displayName || 'member'} on map">
                        ...
                        {#if room.myRoomRole === 'admin' && m.userId !== $authUser?.userId && m.roomRole === 'admin'}
                          <button class="btn-revoke-inline" onclick={stopPropagation(() => revokeAdmin(room.code, m.userId))}>×</button>
                        {/if}
                      </button>
                    {/each}
                  </div>
                ...
                <div class="room-card-actions">
                  {#if room.myRoomRole !== 'admin' && !hasPendingAdminRequest(room)}
                    ...
                    <button class="btn btn-secondary btn-sm tactile" onclick={() => requestAdmin(room.code)}>Request Admin</button>
                      ...
                      <div class="pending-admin-item">
                        ...
                        {#if !par.isMe}
                          <div class="pending-admin-actions">
                            ...
                            {:else}
                              <button class="btn btn-primary btn-xs tactile" onclick={() => voteRoomAdmin(room.code, par.from, 'approve')}>Approve</button>
                              <button class="btn btn-danger btn-xs tactile" onclick={() => voteRoomAdmin(room.code, par.from, 'deny')}>Deny</button>
                            {/if}
                          </div>
                        {/if}
                      </div>
    ...
    <div class="sharing-section">
      ...
      {#if $myContacts.length === 0}
        <EmptyState
          ...
          body="Share your code above with family and friends so you can see each other on the map."
        >
          {#snippet icon()}
            ...
            <Card variant="glass" glow="primary" padding="none">
              <div class="person-card animate-slide-up stagger-item" style="animation-delay:{i*40}ms">
                ...
                <div class="contact-actions-col">
                  <button class="locate-pill" onclick={() => locateContact(c.userId)} title="Find {c.displayName} on map" aria-label="Locate {c.displayName}">
                  ...
                  <button class="btn btn-danger btn-sm tactile" onclick={() => removeContact(c.userId)} disabled={busyAction === 'remove-' + c.userId}>{busyAction === 'remove-' + c.userId ? 'Removing…' : 'Remove'}</button>
                  {#if !isGuardianOf(c.userId) && !isWardOf(c.userId) && !isPendingGuardianOf(c.userId) && !isPendingWardOf(c.userId)}
                    ...
                    <button class="btn btn-secondary btn-sm tactile" onclick={() => requestGuardian(c.userId)} title="You watch them">Watch</button>
                    <button class="btn btn-secondary btn-sm tactile" onclick={() => inviteGuardian(c.userId)} title="They watch you">Be Watched</button>
                  {/if}
                </div>
              </div>
            </Card>
          ...
          {#each $myLiveLinks as link, i}
            ...
            <div class="broadcast-card card-glow-link animate-slide-up stagger-item" style="animation-delay:{i*40}ms">
              ...
              <div class="live-link-actions">
                ...
                <button class="btn btn-danger btn-sm tactile" onclick={() => revokeLink(link.token)}>Stop Sharing</button>
              </div>
            </div>
          {/each}
        </div>
...
<style>
  ...
  /* Section label — bolder, higher-contrast header */
  ...
  /* Magnetic-wrapped primary CTAs keep their intrinsic width in the row */
  :global(.cta-mag) { flex-shrink: 0; }
  :global(.cta-mag) .btn { white-space: nowrap; }

  /* Section empty state (used by Live Sharing) */
  .section-empty {
    display: flex;
    align-items: center;
    gap: var(--space-3);
  ...
  /* ── Person cards (Your People) — inner of glass Card ─────────── */
  .person-card {
    display: flex;
    align-items: center;
    gap: var(--space-3);
  ...
  /* ── Room cards — inner of glass Card ─────────────────────────── */
  .room-card-inner {
    padding: var(--space-3);
    display: flex;
    flex-direction: column;
  ...
  /* ── My Code card — inner of highlighted glass Card ───────────── */
  .my-code-card {
    display: flex;
    align-items: center;
    justify-content: space-between;
  ...
  @media (prefers-reduced-motion: reduce) {
    ...
    .btn-wa:hover,
    .btn-wa:active,
    .quick-action-card:active { transform: none; }
  }
</style>
